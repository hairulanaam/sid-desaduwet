@extends('layouts.app', [
    'title' => 'Galeri Kegiatan Desa Duwet',
    'active' => 'galerikegiatan',
    'page' => 'galerikegiatan',
])
<style>
...
@section('content')
    <section class="relative bg-cover w-full h-[50vh] bg-hero flex"
        ...
        <div class="justify-center items-center mx-auto text-white flex z-10 flex-col">
            <p class="text-5xl font-bold mt-3">Galeri Kegiatan Desa
            ...
            @foreach ($galeriKegiatan as $galeri)
                <div class="flex gap-3 items-center">
                    ...
                    <div class="flex-col space-y-3">
                        <p class="font-semibold text-[22px] text-black text-justify leading-tight">{{ $galeri->judul }}</p>
                        <div class="flex items-center gap-4">
                            <div class="flex gap-1 items-center">
                                <img src="{{ asset('assets/vector/calendar.png') }}" alt="Kalender"
                                    class="h-4 w-auto object-contain">
                                <p class="text-[15px] text-[#b0b0b0]">
                                    {{ \Carbon\Carbon::parse($galeri->tanggal)->translatedFormat('l, d F Y') }}</p>
                            </div>
                            <div class="flex gap-1 items-center">
                                <img src="{{ asset('assets/vector/user.png') }}" alt="Penulis"
                                    class="h-4 w-auto object-contain">
                                <p class="text-[15px] text-[#b0b0b0]">{{ $galeri->penulis }}</p>
                            </div>
                        </div>
                        <p class="text-[15px] text-[#b0b0b0] text-justify">{{ Str::limit($galeri->deskripsi, 100) }}</p>
