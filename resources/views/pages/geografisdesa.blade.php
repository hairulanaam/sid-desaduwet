@extends('layouts.app', [
    'title' => 'Geografis Desa Duwet',
    'active' => 'geografisdesa',
    'page' => 'geografisdesa',
])
...
@section('content')
    <section class="relative bg-cover w-full h-[50vh] bg-hero flex"
        ...
        <div class="justify-center text-center items-center mx-auto text-white flex z-10 flex-col">
            <p class="sm:text-5xl text-2xl font-bold mt-3">Geografis Desa
    ...
    <div class="container mx-auto px-6 py-12 grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2">
            <div class="bg-white shadow-md rounded-lg p-6">
                @php
                    $geografis = \App\Models\GeografisDesa::latest()->first();
                @endphp
                @if ($geografis)
                    <img src="{{ asset('storage/' . $geografis->gambar) }}" alt="Sambutan Desa Duwet"
                    ...
                    @php
                        ...
                        $kalimatArray = preg_split('/(?<=[.!?])\s+/', e($geografis->deskripsi));
                        $paragrafArray = array_chunk($kalimatArray, 4); // Setiap 4 kalimat menjadi 1 paragraf
                    @endphp

                    @foreach ($paragrafArray as $index => $paragraf)
                        <p class="mt-3 mb-4 text-justify leading-relaxed text-base md:text-lg text-gray-600 space-y-4">
                            {!! implode(' ', $paragraf) !!}
                        </p>
                    @endforeach
                    <!-- Container utama -->
                ...
                @else
                    <p class="text-gray-600 text-lg text-center">Belum ada geografis yang tersedia.</p>
                @endif
            </div>
        </div>
