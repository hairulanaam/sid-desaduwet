@extends('layouts.app', [
    'title' => 'Bidang Perikanan Desa Duwet',
    'active' => 'bidangperikanan',
    'page' => 'bidangperikanan',
])
...
@section('content')
    <section class="relative bg-cover w-full h-[50vh] bg-hero flex"
        ...
        <div class="justify-center text-center items-center mx-auto text-white flex z-10 flex-col">
            <p class="sm:text-5xl text-3xl font-bold mt-3">Bidang Perikanan Desa
    ...
    <div class="container mx-auto px-6 py-12 grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-8">
        <!-- KONTEN BIDANG PERIKANAN (KIRI) -->
        <div class="lg:col-span-2">
            <div class="bg-white shadow-md rounded-lg p-6">
                @php
                    $bidangperikanan = \App\Models\BidangPerikanan::latest()->paginate(2); // Pagination 2 item per halaman
                @endphp

                @if ($bidangperikanan->count() > 0)
                    @foreach ($bidangperikanan as $bidang)
                        <div class="mb-8">
                            <!-- Judul -->
                            <h1
                    ...
                    <div class="flex justify-center mt-6">
                        <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center space-x-2">
                            ...
                            <!-- Nomor Halaman -->
                            @foreach ($bidangperikanan->links()->elements[0] as $page => $url)
                                @if ($page == $bidangperikanan->currentPage())
                            ...
                            <!-- Tombol Next -->
                            @if ($bidangperikanan->hasMorePages())
                                <a href="{{ $bidangperikanan->nextPageUrl() }}"
                                    class="px-3 py-2 bg-white border border-gray-300 rounded-md hover:bg-gray-100 transition">
                                    ❯
                                </a>
                            @else
                ...
                @else
                    <p class="text-gray-600 text-lg text-center">Belum ada bidang Perikanan yang tersedia.</p>
                @endif
            </div>
        </div>
