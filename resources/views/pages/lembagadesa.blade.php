    ...
    <section class="py-12 bg-gray-100">
        ...
        <div class="container mx-auto px-6 py-12 grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2">
                @foreach ($lembagaDesa as $lembaga)
                    <div class="bg-white shadow-md rounded-lg p-6 mb-6">
                        <h1 class="uppercase text-xl sm:text-2xl md:text-3xl font-bold text-gray-800 text-center mb-4">
                            {{ $lembaga->judul }}
                        </h1>
                        <h1 class="uppercase text-base sm:text-lg md:text-xl font-bold mb-4 text-gray-800 text-center">
                            {{ $lembaga->deskripsi }}
                        </h1>
            ...
            <div class="w-full absolute left-0 bottom-0 z-50">
                <div
                    class="flex flex-col md:flex-row items-center justify-between bg-white p-3 shadow-lg rounded-md border border-gray-200 text-sm">
                    <!-- Bagian Tags -->
                    <div class="flex items-center gap-2">
                        <p class="text-gray-700 font-semibold text-xs md:text-sm lg:text-base">Tags:</p>
                        <span
                            class="px-2 py-1 bg-gray-100 text-gray-700 font-semibold rounded-md border border-gray-300 shadow-sm text-xs md:text-sm lg:text-base">
                            lembaga desa
                        </span>
                    </div>
                    ...
                    <div class="grid grid-cols-2 md:flex items-center gap-2 mt-3 md:mt-0">
                        <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}"
                            class="flex items-center gap-1.5 bg-[#1877F2] text-white px-3 py-1.5 rounded-md shadow-md text-xs md:text-sm lg:text-base transition-transform transform hover:scale-105">
                            <svg class="w-3 h-3 md:w-4 md:h-4 lg:w-5 lg:h-5 fill-current" viewBox="0 0 24 24">
                                <path
                        ...
                        <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}"
                            class="flex items-center gap-1.5 bg-black text-white px-3 py-1.5 rounded-md shadow-md text-xs md:text-sm lg:text-base transition-transform transform hover:scale-105">
                            <svg class="w-3 h-3 md:w-4 md:h-4 lg:w-5 lg:h-5 fill-current" viewBox="0 0 24 24">
                                <path
                        ...
                        <a href="https://api.whatsapp.com/send?text={{ urlencode(url()->current()) }}"
                            class="flex items-center gap-1.5 bg-[#25D366] text-white px-3 py-1.5 rounded-md shadow-md text-xs md:text-sm lg:text-base transition-transform transform hover:scale-105">
                            <svg class="w-3 h-3 md:w-4 md:h-4 lg:w-5 lg:h-5 fill-current" viewBox="0 0 24 24">
                                <path
                        ...
                        <a href="https://t.me/share/url?url={{ urlencode(url()->current()) }}"
                            class="flex items-center gap-1.5 bg-[#0088cc] text-white px-3 py-1.5 rounded-md shadow-md text-xs md:text-sm lg:text-base transition-transform transform hover:scale-105">
                            <svg class="w-3 h-3 md:w-4 md:h-4 lg:w-5 lg:h-5 fill-current" viewBox="0 0 24 24">
                                <path
