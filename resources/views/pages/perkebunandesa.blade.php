@extends('layouts.app', [
    'title' => 'Bidang Perkebunan Desa Duwet',
    'active' => 'bidangperkebunan',
    'page' => 'bidangperkebunan',
])
...
@section('content')
    <section class="relative bg-cover w-full h-[50vh] bg-hero flex"
        ...
        <div class="justify-center text-center items-center mx-auto text-white flex z-10 flex-col">
            <p class="sm:text-5xl text-3xl font-bold mt-3">Bidang Perkebunan Desa
    ...
    <div class="container mx-auto px-6 py-12 grid grid-cols-1 md:grid-cols-3 gap-8">
        <!-- KONTEN BIDANG PERKEBUNAN  (KIRI) -->
        <div class="md:col-span-2">
            <div class="bg-white shadow-md rounded-lg p-6">
                @php
                    $bidangperkebunan = \App\Models\BidangPerkebunan::latest()->paginate(2); // Pagination 2 item per halaman
                @endphp

                @if ($bidangperkebunan->count() > 0)
                    @foreach ($bidangperkebunan as $bidang)
                        <div class="mb-8">
                            <!-- Judul -->
...
<div class="flex justify-center mt-6">
    <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center space-x-2">
        ...
        <!-- Nomor Halaman -->
        @foreach ($bidangperkebunan->links()->elements[0] as $page => $url)
            @if ($page == $bidangperkebunan->currentPage())
        ...
        <!-- Tombol Next -->
        @if ($bidangperkebunan->hasMorePages())
            <a href="{{ $bidangperkebunan->nextPageUrl() }}"
                class="px-3 py-2 bg-white border border-gray-300 rounded-md hover:bg-gray-100 transition">
                ❯
            </a>
        @else
    ...
    <div class="flex flex-wrap items-center justify-center md:justify-between gap-3">
        ...
        <div class="flex flex-wrap justify-center md:justify-end gap-2 text-center md:text-left">
            ...
            <a href="https://t.me/share/url?url={{ urlencode(url()->current()) }}" 
                ...
                @else
                    <p class="text-gray-600 text-lg text-center">Belum ada bidang Perkebunan yang tersedia.</p>
                @endif
            </div>
        </div>
